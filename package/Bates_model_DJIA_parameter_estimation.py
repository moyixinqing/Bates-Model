...
def obj(p):
    return bmf.BatesObjFunSVC(p,S,rf,q,MktPrice,K,T,PutCall,MktIV,x,w,ObjFun,a,b,Tol,MaxIter)
...
for k in range(NK):
    for t in range(NT):
        ...
        BPrice[k,t] = bmf.BatesPrice(PutCall[k,t],param,T[t],K[k],S,rf,q,x,w);
        BatesIV[k,t] = bmf.BisecBSIV(PutCall[k,t],S,K[k],rf,q,T[t],a,b,BPrice[k,t],Tol,MaxIter);
